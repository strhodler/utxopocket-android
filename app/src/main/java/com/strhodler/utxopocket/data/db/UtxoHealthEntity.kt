...
fun UtxoHealthEntity.toDomain(): UtxoHealthResult =
    UtxoHealthResult(
        txid = txid,
        vout = vout,
        finalScore = finalScore,
...
private fun List<UtxoHealthIndicator>.toIndicatorJsonString(): String {
    val array = JSONArray()
    forEach { indicator ->
        array.put(
            JSONObject().apply {
...
private fun List<UtxoHealthBadge>.toBadgeJsonString(): String {
    val array = JSONArray()
    forEach { badge ->
        array.put(
            JSONObject().apply {
...
private fun Map<UtxoHealthPillar, Int>.toPillarJsonString(): String {
    val obj = JSONObject()
...
private fun String.parseIndicators(): List<UtxoHealthIndicator> =
    runCatching {
        ...
        buildList {
            for (index in 0 until array.length()) {
                ...
                val type = obj.optString("type").takeIf { it.isNotBlank() }
                    ?.let { name -> UtxoHealthIndicatorType.valueOf(name.uppercase(Locale.US)) }
                    ?: continue
                val severity = obj.optString("severity").takeIf { it.isNotBlank() }
                    ?.let { name -> UtxoHealthSeverity.valueOf(name.uppercase(Locale.US)) }
                    ?: UtxoHealthSeverity.LOW
                val delta = obj.optInt("delta")
                val pillar = obj.optString("pillar").takeIf { it.isNotBlank() }
                    ?.let { name -> UtxoHealthPillar.valueOf(name.uppercase(Locale.US)) }
...
private fun String.parseBadges(): List<UtxoHealthBadge> =
    runCatching {
        val array = JSONArray(this)
        buildList {
            for (index in 0 until array.length()) {
                ...
                add(UtxoHealthBadge(id = id, label = label))
            }
        }
    }.getOrElse { emptyList() }

private fun String.parsePillarScores(): Map<UtxoHealthPillar, Int> =
    runCatching {
        val obj = JSONObject(this)
        val scores = mutableMapOf<UtxoHealthPillar, Int>()
        val keys = obj.keys()
        while (keys.hasNext()) {
            val key = keys.next()
            UtxoHealthPillar.values()
        ...
        if (scores.isEmpty()) {
            UtxoHealthPillar.values().associateWithTo(scores) { BASE_SCORE }
        } else {
            UtxoHealthPillar.values()
        ...
        UtxoHealthPillar.values().associateWith { BASE_SCORE }
    }
