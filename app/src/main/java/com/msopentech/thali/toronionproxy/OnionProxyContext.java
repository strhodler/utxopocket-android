...
abstract public class OnionProxyContext {
    ...
    public void installFiles() throws IOException, InterruptedException {
        ...
        try {
            ...
            if (!isInWorkingDirectory(fileTorBin)) {
                ...
                ensureExecutablePermissions(torExecutableFile);
                removeLegacyTorBinary();
            } else {
                torExecutableFile = fileTorBin;
                ensureExecutablePermissions(torExecutableFile);
            }
    ...
    public void setEnvironmentArgsAndWorkingDirectoryForStart(ProcessBuilder processBuilder) {
        ...
        switch (OsData.getOsType()) {
            ...
            case LINUX_64:
                ...
                environment.put("LD_LIBRARY_PATH", buildLdLibraryPath());
                break;
            default:
                break;
        }
    ...
    public String[] getEnvironmentArgsForExec() {
        ...
        switch (OsData.getOsType()) {
            ...
            case LINUX_64:
                ...
                // Linux apparently will not look in the current directory by default. By setting this
                // environment variable we fix that.
                envArgs.add("LD_LIBRARY_PATH=" + buildLdLibraryPath());
                break;
            default:
                break;
        }
    ...
    private boolean isInWorkingDirectory(File fileTorBin) {
        try {
            String workingPath = getWorkingDirectory().getCanonicalPath();
    ...
    private void ensureExecutablePermissions(File torBinary) {
        if (torBinary == null) {
            return;
        }
    ...
    private String buildLdLibraryPath() {
        StringBuilder path = new StringBuilder(getWorkingDirectory().getAbsolutePath());
        File torParent = torExecutableFile != null ? torExecutableFile.getParentFile() : null;
        if (torParent != null) {
            String parentPath = torParent.getAbsolutePath();
            if (!parentPath.equals(getWorkingDirectory().getAbsolutePath())) {
