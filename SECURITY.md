...
- **Calculator camouflage (optional)** — Security Settings can enable a calculator-style challenge before PIN entry. Entering `21000000` then `=` advances to the PIN prompt. This is a UI-only camouflage step: it does not unlock the app, does not change PIN verification/backoff/lockout semantics, and does not alter duress handling.
