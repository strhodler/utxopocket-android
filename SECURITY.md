...
- **Calculator camouflage (optional)** — Security Settings can enable a calculator-style challenge before PIN entry. Entering `21000000` then `=` advances to the PIN prompt. When enabled, launcher icon/label and splash visuals can also present calculator styling. This is still a UI-only camouflage step: it does not unlock the app, does not change PIN verification/backoff/lockout semantics, and does not alter duress handling.
...
- **Camouflage misuse assumptions** — Calculator camouflage is not additional cryptographic authentication; it only adds optional pre-PIN cover traffic for shoulder-surfing scenarios.
